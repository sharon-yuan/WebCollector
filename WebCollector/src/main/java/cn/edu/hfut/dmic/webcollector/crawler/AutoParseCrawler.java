...
public abstract class AutoParseCrawler extends Crawler implements Executor, Visitor, Requester {
	...
	public HttpResponse getResponse(CrawlDatum crawlDatum) throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			java.net.Proxy tempProxy = proxys.nextRandom();
			HttpRequest request = new HttpRequest(crawlDatum, tempProxy);
			result = request.getResponse();
			if (result != null) {

				break;
			} else
				proxys.remove(tempProxy);
	...
	public void execute(CrawlDatum datum, CrawlDatums next) throws Exception {
		System.err.println("execute(CrawlDatum datum, CrawlDatums next) start");
		if (requester == null) {
			System.err.println("AutoParseCrawler.execute requester == null");
			next.add(datum);
			throw new Exception();
		}

		HttpResponse response = requester.getResponse(datum);
		System.err.println("extcute get response");
		Page page = new Page(datum, response);
		System.err.println(response);
		if (response == null) {
			System.err.println("AutoParseCrawler.execute response == null");
			next.add(datum);
			throw new Exception();
		}
